...
const Profile: React.FC = () => {
    ...
    const guesses = userInfo.guess.sort((a: any, b: any) => {
                ...
                <BestGuessesContainer>
                    ...
                    <GridContainer className="leftAlign">
                        {guesses.slice(0, guessesLimit).map((guess: any) => {
                            return <Location id={guess.locationId} className="profileLocation" key={guess.id} image={guess.locationImage} distance={guess.distance} isGuessed={true} />;
                        })}
                    </GridContainer>
                    <LoadMore style={{ width: '200px', marginTop: '20px' }} onClick={() => setGuessesLimit(locationsLimit + 3)}>
                        Load More
                    </LoadMore>
                </BestGuessesContainer>
                ...
                <MyUploadsContainer>
                    ...
                    <GridContainer className="leftAlign">
                        {locations.slice(0, locationsLimit).map((location: any) => {
                            return <Location id={location.id} className="profileLocation" key={location.id} image={location.image} isMyLocation={true} />;
                        })}
                    </GridContainer>
                    <LoadMore style={{ width: '200px', marginTop: '20px' }} onClick={() => setLocationsLimit(locationsLimit + 9)}>
                        Load More
                    </LoadMore>
                </MyUploadsContainer>
            </Container>
